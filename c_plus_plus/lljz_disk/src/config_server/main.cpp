...
int main(int argc, char *argv[])
{
  ...
  const char* sz_pid_file = TBSYS_CONFIG.getString("config_server", "pid_file", "config_server.pid");
  const char* sz_log_file = TBSYS_CONFIG.getString("config_server", "log_file", "config_server.log");
  if(1) {
    char* p, dir_path[256];
  ...
  const char* sz_log_level = TBSYS_CONFIG.getString("config_server", "log_level", "info");
