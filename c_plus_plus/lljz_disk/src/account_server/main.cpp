...
lljz::disk::AccountServer* acc_srv = NULL;
...
/*  if(tbsys::CProcess::startDaemon(sz_pid_file, sz_log_file) == 0)*/ {
    ...
    acc_srv = new lljz::disk::AccountServer();
    acc_srv->Start();

    // ignore signal when destroy, cause sig_handler may use acc_srv between delete and set it to NULL.
