...
public class RepaymentMissionLoan {
	public List<DispatchInfo> dispatchs;
	
	public RepaymentMissionLoan() {
	...
	public void doJob() {
		...
		String sql="select id,title,status,borrower_id,borrower_name,amount,annual_rate,periods,repay_mode from mission_loans where status in (6)";
	...
	public void checkRepayments(Connection conn,MissionLoans missionLoan,List<Repayment> repayments) {
		QueryRunner qr=new QueryRunner();
		String sql=String.format("select id,loan_id,status,period,repay_mode,still_principal,still_interest from mission_repayment where loan_id='%s'",
				missionLoan.id);
	...
	public void saveFile() {
		...
		try {
			if (fileDispatch.length() > 0) {
				File w=new File("D:\\job_projects\\python\\financial_plan_dispatch_money.dat");
			...
			e.printStackTrace();
			System.out.println("保存文件失败-理财计划散标还款修复，D:\\job_projects\\python\\financial_plan_dispatch_money.dat");
			Tool.exitSystem();
		}
		...
		try {
			if (sumFileDispatch.length() > 0) {
				File w=new File("D:\\job_projects\\python\\financial_plan_sum_dispatch_money.dat");
				   FileWriter fw = new FileWriter(w.getAbsoluteFile());
				   BufferedWriter bw = new BufferedWriter(fw);
			...
			e.printStackTrace();
			System.out.println("保存文件失败-理财计划散标还款修复，D:\\job_projects\\python\\financial_plan_sum_dispatch_money.dat");
			Tool.exitSystem();
		}
		...
		try {
			if (fileFixPrincipalSqls.length() > 0) {
				File w=new File("D:\\job_projects\\python\\financial_plan_fix_principal_sqls.dat");
				   FileWriter fw = new FileWriter(w.getAbsoluteFile());
				   BufferedWriter bw = new BufferedWriter(fw);
			...
			e.printStackTrace();
			System.out.println("保存文件失败-理财计划散标还款修复，D:\\job_projects\\python\\financial_plan_fix_principal_sqls.dat");
			Tool.exitSystem();
		}

		try {
			if (fileFixInterestSqls.length() > 0) {
				File w=new File("D:\\job_projects\\python\\financial_plan_fix_interest_sqls.dat");
				   FileWriter fw = new FileWriter(w.getAbsoluteFile());
				   BufferedWriter bw = new BufferedWriter(fw);
			...
			e.printStackTrace();
			System.out.println("保存文件失败-理财计划散标还款修复，D:\\job_projects\\python\\financial_plan_fix_interest_sqls.dat");
			Tool.exitSystem();
		}
	}
