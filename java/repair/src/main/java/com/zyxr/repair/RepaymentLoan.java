...
public class RepaymentLoan {
	...
	public void doJob() {
		...
		try {
			loans = qr.query(conn, sql, handler);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void checkRepayments(Connection conn,Loans loan,List<Repayment> repayments) {
		QueryRunner qr=new QueryRunner();
		String sql=String.format("select id,loansid,status,period,repaymode,stillPrincipal,stillInterest from repayment where loansid='%s'",
				loan.id);
